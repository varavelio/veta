...
- `internal/template` owns Pongo2 template loading, template-name resolution, rendering, and filter registration. Templates and components can include shared Pongo fragments from `includes/` through normal Pongo include statements. It must not know about pages, data loading, themes, Markdown, components, or output files.
