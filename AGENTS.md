...
- `internal/config` owns loading Veta tool configuration only, including build behavior such as output directory and clean mode, development server settings such as host, port, and additional watch paths, format-specific settings such as HTML and Tailwind minification, and theme source. Site content, theme data, SEO metadata, navigation, and other user-facing values belong outside `veta.yaml`.
- `internal/data` owns loading global files from `data/` only. It supports nested directories, JSON/YAML/TOML/JS inputs, returns JSON-compatible values keyed by the directory/file stem path, and lets higher-priority filesystem layers replace lower-priority files with the same extensionless key while rejecting duplicates within one layer. It must not render templates, discover pages, parse Markdown, or write output files.
...
- `internal/js` owns synchronous execution of self-contained Veta JavaScript files and the explicit runtime context object passed to default exports. Its file and HTTP APIs should return text, with explicit parsing and rendering exposed through `context.parse`; Markdown and component rendering must remain narrow injected interfaces implemented by their owning packages. File APIs read from an injected composed filesystem when one is provided, so theme files are visible alongside project files, and fall back to the physical project root otherwise. It must not expose runtime APIs through global JavaScript variables or become a module loader, template engine, page loader, or output writer.
- `internal/sourcefile` owns the shared filename whitelist for automatically discovered project source files. It accepts one extension separator with ASCII alphanumeric or underscore characters on both sides, and must not own directory traversal, supported-extension policy, parsing, or file loading.
...
- `internal/markdown` owns Markdown-to-HTML conversion only. It preserves trusted raw HTML, including HTML-like opening tags continued across lines, without knowing which tags represent components. It must not know about pages, templates, filters, components, themes, data loading, or output files.
- `internal/loaddata` owns loading one local or remote text source for template `load_data`, including path/URL validation and HTTP GET. It must not parse structured formats, discover project-wide data, render templates, execute JavaScript, discover pages, process Markdown, or write output files.
- `internal/parsecontent` owns explicit parsing of text into JSON-compatible values for JSON, YAML, TOML, and Markdown frontmatter. It must not read files, fetch URLs, render Markdown to HTML, execute JavaScript, discover project files, or write output files.
...
- `internal/components` owns component discovery and bounded source rewriting only. It discovers component templates from `components/`, parses registered tags in content while preserving code and non-tag HTML contexts, supports slots through injected renderers, and must not render Markdown, own template engines, discover pages, load data, or write output files.
- `internal/filters` owns native filter definitions, including explicit parse filters, and loading JavaScript filter sources from `filters/` only. It keeps the directory flat, exposes filters through package-local interfaces, and must not import JavaScript runtimes, template engines, page loaders, or output writers directly.
- `internal/functions` owns built-in template functions and loading JavaScript template functions from `functions/` only. It keeps the directory flat, injects context-bound functions into render/component contexts, and must not own template rendering, page discovery, data loading beyond the `load_data` adapter, JavaScript runtime construction, or output writing.
...
- `internal/template` owns Pongo2 template loading, template-name resolution, rendering, filter/global registration, and the Pongo-facing `load_data` function adapter. Page templates and components share the normal Pongo loader for includes, inheritance, and macro imports; Veta does not prescribe subdirectories for supporting files under `templates/`. It must not own data loading, discover pages, resolve themes, process Markdown, discover components, or write output files.
- `internal/render` owns composing one page into one output document. It exposes the root template context `{ data, pages, page, props }`, where `pages` is the flat normalized page list and `page` is the current page. Pages without `template` emit raw `content`; pages with `template` receive generator-provided `content` unchanged and trusted. Explicit Markdown and component rendering belongs at the JavaScript API boundary, not in the document renderer. The package depends on injected interfaces for templates and must not discover files, load data, load filters, process themes, copy assets, or write output files.
...
- `internal/dev` owns the development-only server workflow: fresh temporary build output per build with atomic swap, cancelable full clean rebuild orchestration through `internal/build` (a change during a build cancels it and queues a fresh rebuild), polling-based project watching, local HTTP serving, and revision-based polling live reload instead of long-lived SSE connections. It must not become a production server, write to configured production output directories, duplicate build logic, or move CLI flag parsing out of `internal/cli`.
- `internal/cli` owns command selection, flag parsing, help text, human-facing command errors, and delegation to application workflows. Build behavior settings such as output directory and clean mode belong in `veta.yaml`, not the CLI. It must not load site files, render content, resolve themes, scaffold starter files, or write build output directly.
