...
- `internal/loaddata` owns loading one local or remote data source for template `load_data`, including text/JSON/YAML/TOML parsing, path/URL validation, HTTP GET, and JSON-compatible normalization. It must not discover project-wide data, render templates, execute JavaScript, discover pages, process Markdown, or write output files.
...
- `internal/template` owns Pongo2 template loading, template-name resolution, rendering, filter/global registration, and the Pongo-facing `load_data` tag/function adapter. Templates and components can include shared Pongo fragments from `includes/` through normal Pongo include statements. It must not own data loading, discover pages, resolve themes, process Markdown, discover components, or write output files.
