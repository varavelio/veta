...
- `internal/template` owns Pongo2 template loading, template-name resolution, rendering, filter/global registration, and the Pongo-facing `load_data` function adapter. Templates and components can include shared Pongo fragments from `includes/` through normal Pongo include statements. It must not own data loading, discover pages, resolve themes, process Markdown, discover components, or write output files.
