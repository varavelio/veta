...
- `internal/config` owns loading Veta tool configuration only, including build behavior such as output directory, clean mode, debug mode, development server settings such as host, port, and additional watch paths, format-specific settings such as HTML and Tailwind minification, and theme source. Site content, theme data, SEO metadata, navigation, and other user-facing values belong outside `veta.yaml`.
